...
GMotor::GMotor(GM_driverType type, int8_t param1, int8_t param2, int8_t param3, int8_t param4) {
	...
	if (_pwmD != _GM_NC) pinMode(_pwmD, OUTPUT);
	setMode(STOP);
}
...
void GMotor::run(GM_workMode mode, int16_t duty) {
	...
	if (_deadtime > 0 && _lastMode != mode) {
		_lastMode = mode;
		setPins(_level, _level, 0, 0);	// выключить всё
	...
	switch (mode) {
	case FORWARD:	setPins(_level, !_level, duty, _level* 255); _state = 1; break;		
	case BACKWARD:	setPins(!_level, _level, _level* 255, (_type == DRIVER2WIRE) ? (_maxDuty - duty) : (duty)); _state = -1; break;
	case BRAKE:		setPins(!_level, !_level, !_level * 255, !_level * 255); _state = 0; break;	// при 0/255 analogWrite сделает 0/1
	case STOP:		setPins(_level, _level, _level * 255, !_level * 255); _state = 0; break;		
	}
}

void GMotor::setPins(bool a, bool b, int c, int d) {
	if (_digA != _GM_NC) digitalWrite(_digA, a);
	if (_digB != _GM_NC) digitalWrite(_digB, b);
	if (_pwmC != _GM_NC) analogWrite(_pwmC, c);
	if (_pwmD != _GM_NC) analogWrite(_pwmD, d);
}
